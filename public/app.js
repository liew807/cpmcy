class cpmcylone {
    constructor() {
        ...
        this.startTime = null;
        this.currentUser = null; // 新增：当前登录用户信息
        console.log('cpmcy Clone 初始化成功. 基础URL:', this.baseUrl);
    }

    init(userInfo = null) {
        this.currentUser = userInfo; // 保存用户信息
        this.bindEvents();
    ...
    setupUserPermissions() {
        ...
        if (this.currentUser.cardType === 'hour') {
            ...
            if (option2Radio) {
                ...
                document.getElementById('op-type1').checked = true;
            }
    ...
    bindEvents() {
        ...
        const logoutBtnSystem = document.getElementById('logout-btn');
...
// 修改：等待验证系统完成后初始化
window.cpmcyCloneApp = null;

// 添加一个函数供验证系统调用
function initCPMClone(userInfo) {
...
// 导出函数供验证系统调用
window.initCPMClone = initCPMClone;
