...
const DATA_FILE = process.env.RENDER 
    ? '/opt/render/.data/data.json'  // Render持久化磁盘路径
    : path.join(__dirname, 'data.json');  // 本地开发路径
...
async function initData() {
    try {
        // 如果是Render平台，确保持久化目录存在
        if (process.env.RENDER) {
            const diskPath = '/opt/render/.data';
            try {
                await fs.access(diskPath);
                console.log('✅ 持久化目录已存在:', diskPath);
            } catch (error) {
                console.log('📁 创建持久化目录:', diskPath);
...
app.get('/api/status', async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: {
                status: 'running',
                storageType: process.env.RENDER ? 'Render持久化磁盘' : '本地文件',
...
async function startServer() {
    ...
    app.listen(PORT, () => {
        ...
        📍 平台: ${process.env.RENDER ? 'Render' : '本地'}
        📍 存储: ${DATA_FILE}
